...
hist(sus.age$NO.OF.PEOPLE, main = "Distribution of people who suspected Covid-19 in different age groups",
     xlab = "No.of.People", col = "cyan3",border = "white")
```
...
```{r}
male.10.symptom = subset(top.10.g,select = c("DESCRIPTION","M"))
male.10.symptom = na.omit(male.10.symptom)
kable(male.10.symptom,caption = "Top 10 symptoms in male covid patients")

ggplot(male.10.symptom, aes(x = DESCRIPTION,y = M, fill = M)) +
  geom_bar(stat = "identity",fill = "cyan4",colour= "white") +
  labs(title = "Top 10 symptoms in male covid patients",x = "Symptoms",
       y = "Count") +
  theme_bw()+coord_flip()
...
```{r}
female.10.symptom = subset(top.10.g,select = c("DESCRIPTION","F"))
...
ggplot(female.10.symptom, aes(x = DESCRIPTION,y = F, fill = F)) +
  ...
  labs(title = "Top 10 symptoms in female covid patients",x = "Symptoms",
       y = "Count") +
  theme_bw()+coord_flip()
...
ggplot(Qn3.2, aes(x = ENCOUNTERCLASS, fill = GENDER)) +
  ...
  labs(title = "ENCOUNTERCLASS BETWEEN GENDERS") + 
  theme_bw()+ scale_fill_manual(values = c("lightpink3","skyblue"))
